...
control.pal <- brewer.pal(3, "Blues")
treat.pal <- brewer.pal(3, "Reds")
level.pal <- c(control.pal, treat.pal)
...
if(colnames(rma.comp)[1] != "PROBEID") rma.comp <- rma.comp[, c(25, 1:24)]
head(rma.comp)
...
if(nrow(my.rma) == length(present)) my.rma <- my.rma[present >= 4, ]
...
sample.no <- 1:24
plotMDS(exprs(my.rma)[, sample.no], labels=pData(my.rma)$sample.labels[sample.no], top=500, 
...
my.contrasts <- makeContrasts(C24vC06 = C.24 - C.06, C48vC06 = C.48 - C.06, C48vC24= C.48 - C.24, 
                              T24vT06 = T.24 - T.06, T48vT06 = T.48 - T.06, T48vT24 = T.48 - T.24, 
                              T06vC06 = T.06 - C.06, T24vC24 = T.24 - C.24, T48vC48 = T.48 - C.48, 
                              levels = my.design)
my.contrasts
...
single.gene <- select(hgu133plus2.db, keys=rownames(contrast.tts[["C24vC06"]])[1], keytype="PROBEID", 
                    columns=c("ACCNUM", "SYMBOL", "ALIAS"))
dim(single.gene)
head(single.gene)

gene.data <- select(hgu133plus2.db, keys=rownames(contrast.tts[["C24vC06"]]), keytype="PROBEID", 
